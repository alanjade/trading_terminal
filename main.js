...
import { loadSettings, saveSettings, loadWatchlist, saveWatchlist, loadCollapsed, saveCollapsed,
  saveLockedSuggestion, loadLockedSuggestion } from './state/persistence.js';
...
import { calcFuturesMetrics, calcRiskBasedSize, calcATRStop, calcATRTrailStop, calcDailyGoal, suggestLeverage, MM_TIERS } from './engine/risk.js';
import { fetchRiskTiers } from './services/riskTiers.js';
import { fetchFundingRate } from './services/funding.js';
...
import { initJournal, openJournalEntry, saveJournalEntry, renderJournalList, renderJournalStats, exportJournal, deleteJournalTrade, editJournalTrade,
  logAutoTrade, closeAutoTrade, deleteAutoTrade } from './components/journal.js';
...
export async function initSym(sym, tf) {
  ...
  // Restore any locked suggestion the user was tracking for this sym/tf
  // before a reload or symbol switch. Validity (stop-out) is re-checked
  // against fresh live price the moment the next tick lands.
  state.lockedSuggestion = loadLockedSuggestion(sym, tf);
  _refreshRiskTiers(sym, state.exchange);
  _refreshFunding(sym, state.exchange);
...
// ── Locked Suggestion ─────────────────────────────────────────────────────────
// The raw `computeSuggestion()` output re-evaluates on every recompute (candle
// close / symbol switch) and can flip its `entry` around with price. Traders
// want a stable "this is the trade I'm tracking" entry instead. We only
// (re)lock a new suggestion when:
//   1. nothing is locked yet,
//   2. the locked setup's direction changed AND the new dir has been proposed
//      by computeSuggestion (i.e. a real regime/EMA flip, not just recompute noise), or
//   3. price has traded through the locked stop (setup invalidated).
// Otherwise the locked entry/stop/target stay put; the raw `sug` is still
// shown separately as the "live" read so nothing is hidden from the user.

// Manual pin/unpin — lets the user freeze *any* currently-displayed
// suggestion (auto or already-locked) or release it back to auto mode.
// ── Funding Rate ──────────────────────────────────────────────────────────
let _fundingSeq = 0;
async function _refreshFunding(sym, exchange) {
  const seq = ++_fundingSeq;
  try {
    const data = await fetchFundingRate(exchange, sym);
    if (seq !== _fundingSeq) return; // sym/exchange changed while fetching
    state.fundingInfo = data;
    updateFundingUI();
  } catch(e) { /* leave stale/null */ }
}

function updateFundingUI() {
  const info = state.fundingInfo;
  const rateEl = dom.lazy('fee-funding-rate');
  const timeEl = dom.lazy('fee-funding-time');
  const costEl = dom.lazy('fee-funding-cost');
  if (!info) {
  ...
  if (rateEl) {
    rateEl.textContent = (info.rate >= 0 ? '+' : '') + pct;
    rateEl.style.color = info.rate >= 0 ? 'var(--red)' : 'var(--green)'; // positive = longs pay
  }
  if (timeEl && info.nextFundingTime) {
    const msLeft = info.nextFundingTime - Date.now();
    if (msLeft > 0) {
  ...
  if (costEl && state.liveRiskTiers !== undefined) {
    const posSize = state._lastPosSize || 0;
    const dirSign = state.currentDir === 'long' ? 1 : -1;
    const cost = posSize * info.rate * dirSign;
    costEl.textContent = (cost >= 0 ? '-$' : '+$') + Math.abs(cost).toFixed(3);
    costEl.style.color = cost >= 0 ? 'var(--red)' : 'var(--green)';
  }
}

// Refresh the funding countdown display every 30s without re-fetching the
...
async function checkMTFAgreement() {
  const badge = document.getElementById('mtf-agree-badge');
  if (!badge) return;
  ...
  const tfs  = ['15m', '1h', '4h'];
  try {
    const data = await batchFetchScreener([sym], tfs, state.exchange);
    const tfCandles = data.get(sym) || {};
    const primaryTf = tfs.find(t => tfCandles[t]?.length >= 15);
    if (!primaryTf) { badge.textContent = '⇄ Not enough data'; return; }

    const snapshotMap = {};
    tfs.forEach(t => {
      const c = tfCandles[t];
      if (c?.length) { const s = calcTFSnapshot(c, t); if (s) snapshotMap[t] = s; }
    });

    const r = analyseSymbol(sym, tfCandles[primaryTf], primaryTf, snapshotMap, tfs, Date.now(), state.exchange);
    if (!r) { badge.textContent = '⇄ Check MTF agreement'; return; }

    const agreeCount = Math.max(r.bullCount, r.bearCount);
    const dirLabel = r.mtfDir === 'bull' ? 'LONG' : r.mtfDir === 'bear' ? 'SHORT' : 'mixed';
    const color = r.mtfFull ? 'var(--green)' : r.mtfMost ? 'var(--amber)' : 'var(--text3)';
    badge.style.color = color;
    badge.textContent = `⇄ ${agreeCount}/${r.availTFs} TFs agree: ${dirLabel} (${r.mtfScore}%)`;
  } catch(e) {
    badge.textContent = '⇄ Check failed — try again';
  }
}

function _checkStopLiquidityZone(sug, atr) {
  const el = dom.lazy('liq-zone-warn');
  if (!el) return;
  if (!sug?.stop || !atr) { el.style.display = 'none'; return; }

  const tolerance = atr * 0.35;
  const sweeps = (state.liquiditySweeps || []).filter(s => s.recencyBars <= 15);
  const equals = (state.equalLevels    || []).filter(l => l.count >= 2);

  const hitSweep = sweeps.find(s => Math.abs(s.swingPrice - sug.stop) <= tolerance);
  const hitEqual = equals.find(l => Math.abs(l.price - sug.stop) <= tolerance);

  if (hitSweep) {
    el.textContent = `⚠️ Stop sits near a recent liquidity sweep (${hitSweep.swingType} @ ${fmt(hitSweep.swingPrice)}) — classic stop-hunt zone.`;
    el.style.display = 'block';
  } else if (hitEqual) {
    el.textContent = `⚠️ Stop sits inside an equal-${hitEqual.type} cluster (${hitEqual.label}) — a known liquidity pocket.`;
    el.style.display = 'block';
  } else {
...
function renderQualitySparkline() {
  const svg = document.getElementById('quality-sparkline');
  if (!svg) return;
  const hist = state.qualityHistory;
  if (!hist.length) { svg.innerHTML = ''; return; }

  const w = 44, h = 16, pad = 1;
  const min = 0, max = 100; // score is always 0-100, fixed scale so the line's height is meaningful trade-to-trade
  ...
  svg.innerHTML = `<polyline points="${pts}" fill="none" stroke="${color}" stroke-width="1.2"/>`;
}

function toggleTierTable() {
  const el = dom.lazy('liq-tier-table');
  if (!el) return;
  if (el.style.display !== 'none') { el.style.display = 'none'; return; }

  const tiers = state.liveRiskTiers?.length ? state.liveRiskTiers : (MM_TIERS[state.exchange] || MM_TIERS.binance);
  const rows = tiers.map(t =>
    `≤ $${t.max === Infinity ? '∞' : t.max.toLocaleString()} notional — mm ${(t.mmRate * 100).toFixed(2)}%`
  ).join('<br>');
  el.innerHTML = `<b>${state.liveRiskTiers?.length ? 'Live' : 'Static estimate'} tiers (${state.exchange}):</b><br>${rows}`;
  el.style.display = 'block';
}


// ── Live Risk Tiers ────────────────────────────────────────────────────────
// Non-blocking: fetches real per-symbol maintenance-margin tiers (Bybit/OKX
// public APIs; Binance falls back to the static table — see riskTiers.js).
// Results land in state.liveRiskTiers and feed calcFuturesMetrics on the
// next render; nothing blocks on this, so the UI shows the static-tier
// estimate immediately and quietly upgrades to the live one once it lands.
let _riskTiersSeq = 0;
async function _refreshRiskTiers(sym, exchange) {
  const seq = ++_riskTiersSeq;
  state.liveRiskTiers = null;
  try {
    const tiers = await fetchRiskTiers(exchange, sym);
    if (seq !== _riskTiersSeq) return; // sym/exchange changed again while fetching
    if (tiers?.length) {
      state.liveRiskTiers = tiers;
      computeAndRender();
    }
...
function togglePinSuggestion() {
  const locked = state.lockedSuggestion;
  const btn = dom.lazy('sug-pin-btn');

  if (locked?.manual) {
    // Unpin: fall back to auto-lock behavior starting from the current raw read.
    state.lockedSuggestion = state.suggestion ? { ...state.suggestion, lockedAt: Date.now() } : null;
    saveLockedSuggestion(state.sym, state.tf, state.lockedSuggestion);
    if (btn) { btn.style.background = 'var(--bg3)'; btn.style.color = 'var(--text3)'; btn.title = 'Pin this entry'; }
    showToast?.('Suggestion unpinned — auto-tracking resumed');
  } else {
    const base = locked || state.suggestion;
    if (!base) return;
    state.lockedSuggestion = { ...base, manual: true, lockedAt: locked?.lockedAt || Date.now() };
    saveLockedSuggestion(state.sym, state.tf, state.lockedSuggestion);
    if (btn) { btn.style.background = 'var(--yellow, #eab308)'; btn.style.color = '#000'; btn.title = 'Unpin — resume auto-tracking'; }
    showToast?.('Suggestion pinned — won\'t auto-update');
  }
  computeAndRender();
  updateTrackButtonUI();
}

// Logs the currently-locked suggestion to the journal and remembers its
// trackingId so a later stop/target hit can auto-fill the exit for it.
// Toggling again while still open removes it from the journal (undo).
function toggleTrackSuggestion() {
  const locked = state.lockedSuggestion;
  if (!locked) { showToast?.('No active suggestion to track'); return; }

  if (locked.tracked) {
    // Untrack: delete the still-open journal entry rather than leaving an
    // orphaned unclosed trade behind. No confirm dialog — the click itself
    // is the decision.
    if (locked.trackingId) deleteAutoTrade(locked.trackingId);
    locked.tracked = false;
    locked.trackingId = null;
    showToast?.('Stopped tracking — removed from journal');
  } else {
    const trade = logAutoTrade({
      sym: state.sym, dir: locked.dir, tf: state.tf,
      entry: locked.entry, stop: locked.stop, target: locked.target,
      regime: state.regime, score: state.suggestion?.score,
      trackingId: locked.lockedAt || Date.now(),
    });
    locked.tracked = true;
    locked.trackingId = trade.trackingId;
    showToast?.('Tracking — will auto-log the result on stop/target');
  }
  saveLockedSuggestion(state.sym, state.tf, locked);
  updateTrackButtonUI();
}

function updateTrackButtonUI() {
  const btn = dom.lazy('sug-track-btn');
  if (!btn) return;
  const locked = state.lockedSuggestion;
  if (locked?.tracked) {
    btn.style.background = 'var(--green)'; btn.style.color = '#000';
    btn.textContent = '● Tracking'; btn.title = 'Stop tracking (removes open journal entry)';
  } else {
    btn.style.background = 'var(--bg3)'; btn.style.color = 'var(--text3)';
    btn.textContent = '▶ Track'; btn.title = 'Log this trade to the journal and auto-close it on stop/target';
  }
}

function _updateLockedSuggestion(sug) {
  if (!sug) return;
  const locked = state.lockedSuggestion;

  if (!locked) {
    state.lockedSuggestion = { ...sug, lockedAt: Date.now() };
    saveLockedSuggestion(state.sym, state.tf, state.lockedSuggestion);
    return;
  }

  const invalidated = locked.dir === 'long'
    ? state.livePrice <= locked.stop
    : state.livePrice >= locked.stop;

  const targetHit = locked.target && (locked.dir === 'long'
    ? state.livePrice >= locked.target
    : state.livePrice <= locked.target);

  const directionChanged = sug.dir !== locked.dir;

  if (!invalidated && !targetHit && !directionChanged) return;

  // Always alert on stop-out, even for a manual pin — the trader still
  // needs to know the setup broke, they just decide what happens next.
  if (invalidated && !locked._alerted) {
    const msg = `⚠️ ${fmtSym(state.sym)} setup stopped out at ${fmt(state.livePrice)}`;
    showToast(msg, 'toast-warn');
    playBeep?.(440);
    if (Notification?.permission === 'granted') new Notification('TradeAssist', { body: msg });
    locked._alerted = true; // avoid re-firing every tick while price stays past stop
  }

  if (targetHit && !locked._tpAlerted) {
    const msg = `🎯 ${fmtSym(state.sym)} setup hit target at ${fmt(state.livePrice)}`;
    showToast(msg);
    playBeep?.(880);
    if (Notification?.permission === 'granted') new Notification('TradeAssist', { body: msg });
    locked._tpAlerted = true;
  }

  // Auto-close the journal entry if this locked suggestion was being tracked.
  if (locked.tracked && locked.trackingId && (invalidated || targetHit)) {
    const exitPrice = invalidated ? locked.stop : locked.target;
    const result = invalidated ? 'loss' : 'win';
    closeAutoTrade(locked.trackingId, exitPrice, result);
    locked.tracked = false;
    updateTrackButtonUI();
  }

  // Manually pinned suggestions are never auto-replaced.
...
function _computeSignalsAndUI(all, atr, atrPct = 0) {
  ...
  // Track score history for the sparkline (capped, reset on symbol switch).
  if (typeof quality?.score === 'number') {
    state.qualityHistory.push(quality.score);
    if (state.qualityHistory.length > 40) state.qualityHistory.shift();
  }
  renderQualitySparkline();

  // TPs / sizing / futures math track the LOCKED suggestion (the trade
  // idea the user is actually watching), not the raw re-evaluated one.
  const activeSug = state.lockedSuggestion || sug;

  _checkStopLiquidityZone(activeSug, atr);

  const tps       = computePartialTPs({ entry: activeSug?.entry, stop: activeSug?.stop, dir: activeSug?.dir });
  const trailStop = calcATRTrailStop(state.livePrice, atr, state.currentDir, 2);
  const atrSize   = atr ? calcAtrPositionSize({ capital, riskPct: 1, entry: activeSug?.entry || state.livePrice, atr, atrMultiple: 2 }) : null;

  const entry = (entryRaw !== '' && +entryRaw) ? +entryRaw : (activeSug?.entry || state.livePrice);
  const stop  = (stopRaw  !== '' && +stopRaw)  ? +stopRaw  : (activeSug?.stop  || 0);
  ...
  const futMetrics = calcFuturesMetrics({
    ...
    exchange: state.exchange, customTiers: state.liveRiskTiers,
  });
  state._lastPosSize = futMetrics.posSize;
  updateFundingUI();

  updateSuggestionUI(state.lockedSuggestion, quality, tps, trailStop, atrSize, _lastBTResult, sug);
  updateTrackButtonUI();
...
function drawAll() {
  ...
  const chartSug = state.lockedSuggestion || state.suggestion;
  if (chartSug?.entry) _lwcChart.setSuggestion(chartSug);
...
function updateSuggestionUI(sug, quality, tps, trailStop, atrSize, btResult, liveSug) {
  if (!sug) return;
  ...
  // Primary display = LOCKED entry (stable trade idea).
  ...
  // Secondary: live re-evaluated price, shown as a delta so the locked
  // entry never gets silently overwritten while the setup is still valid.
  const liveEl = dom.lazy('sug-live-delta');
  if (liveEl) {
    if (liveSug && sug.entry) {
      const diffPct = ((state.livePrice - sug.entry) / sug.entry) * 100;
      const sign    = diffPct >= 0 ? '+' : '';
      const drifted = Math.abs(diffPct) >= 0.05;
      liveEl.style.display = drifted ? '' : 'none';
      liveEl.textContent   = drifted
        ? `live ${fmt(state.livePrice)} (${sign}${diffPct.toFixed(2)}%)`
        : '';
      liveEl.style.color = diffPct >= 0
        ? (sug.dir === 'long' ? 'var(--green)' : 'var(--red)')
        : (sug.dir === 'long' ? 'var(--red)' : 'var(--green)');
    } else {
  ...
  const ageEl = dom.lazy('sug-locked-age');
  if (ageEl && sug.lockedAt) {
    const mins = Math.max(0, Math.round((Date.now() - sug.lockedAt) / 60000));
    ageEl.textContent = sug.manual
      ? '(pinned)'
      : (mins < 1 ? '(just now)' : `(locked ${mins}m ago)`);
  }

  const pinBtn = dom.lazy('sug-pin-btn');
  if (pinBtn) {
    if (sug.manual) {
      pinBtn.style.background = 'var(--yellow, #eab308)';
      pinBtn.style.color = '#000';
      pinBtn.title = 'Unpin — resume auto-tracking';
    } else {
      pinBtn.style.background = 'var(--bg3)';
      pinBtn.style.color = 'var(--text3)';
      pinBtn.title = 'Pin this entry';
    }
  }
...
function updateFuturesUI(metrics, leverage, entry) {
  ...
  const tierBadge = dom.lazy('liq-tier-badge');
  if (tierBadge) {
    tierBadge.textContent = state.liveRiskTiers?.length ? '(live)' : '(est.)';
    tierBadge.style.color = state.liveRiskTiers?.length ? 'var(--green)' : 'var(--text3)';
  }
